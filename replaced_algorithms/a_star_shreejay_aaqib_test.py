...
# Action1 - moving robot to a step size at 30deg
def action2(pop):
    x, y, theta = pop[0]
    cost_to_come = pop[1][2]
...
# Action1 - moving robot to a step size at 0deg
def action3(pop):
    x, y, theta = pop[0]
    cost_to_come = pop[1][2]
...
# Action1 - moving robot to a step size at 330deg
def action4(pop):
    x, y, theta = pop[0]
    cost_to_come = pop[1][2]
    theta_new = custom_ang_round(theta - 30)
...
# Action1 - moving robot to a step size at 300deg
def action5(pop):
    x, y, theta = pop[0]
    cost_to_come = pop[1][2]
    theta_new = custom_ang_round(theta - 60)
...
all_obstacles = obstacles(obstacle_buffer,robot_size)
...
# x_f = custom_coord_round(50)
# y_f = custom_coord_round(10)
...
while not Done:
    ...
    pygame.draw.circle(monitor, (50, 150, 140), to_pygame(H6, 250), 5)
    pygame.draw.circle(monitor, (50, 150, 140), to_pygame(H5, 250), 5)
    ...
    for l in range(len(visited_nodes_track) - 2):
        ...
        arrow(monitor, "white", (255, 0, 0),[m[0], m[1]], [n[0], n[1]], 0.5)
        pygame.display.flip()
        clock.tick(300)
    for i in the_path:
